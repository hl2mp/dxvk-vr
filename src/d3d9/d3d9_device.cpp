...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::CreateTexture(
    ...
    GetVRSystem()->GetRenderTargetSize(rtWidth, rtHeight);

    // we are creating a vr rendertarget
    if (index != -1 && Width == rtWidth && Height == rtHeight && GetVRSystem()->IsMultiSampleEnabled())
    {
  ...
  HRESULT STDMETHODCALLTYPE D3D9DeviceEx::CreateDepthStencilSurfaceEx(
    ...
    // we are creating a vr rendertarget
    if ( Width == rtWidth && Height == rtHeight && GetVRSystem()->IsMultiSampleEnabled() )
    {
        MultisampleQuality = m_implicitSwapchain->GetPresentParams()->MultiSampleQuality;
